...
Após configurar, você terá análises inteligentes e específicas para cada código! 🚀
            `,timestamp:new Date().toISOString(),isDemoFeedback:!0}}getFileType(e){const s=e.split(".").pop().toLowerCase();return{js:"javascript",jsx:"javascript",ts:"typescript",tsx:"typescript",py:"python",java:"java",cpp:"cpp",c:"c",cs:"csharp",php:"php",rb:"ruby",go:"go",rs:"rust",html:"html",css:"css",scss:"scss",sass:"sass",json:"json",xml:"xml",md:"markdown",txt:"text",sql:"sql"}[s]||"text"}isTextFile(e){const s=["js","jsx","ts","tsx","py","java","cpp","c","cs","php","rb","go","rs","html","css","scss","sass","json","xml","md","txt","sql","yml","yaml","ini","conf","config","log","csv","sh","bat","ps1"],o=e.split(".").pop().toLowerCase();return s.includes(o)}}const T=new N;class R{constructor(){this.modal=null,this.isOpen=!1}show(e,s={}){this.create(e,s),this.open()}create(e,s){this.destroy(),this.modal=document.createElement("div"),this.modal.className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black bg-opacity-50 feedback-modal-overlay",this.modal.onclick=a=>{a.target===this.modal&&this.close()};const o=document.createElement("div");o.className=`
    ...
    `,x.appendChild(v),d.length>0){const i=document.getElementById("pending-missions-list");d.forEach(p=>{const l=s.find(b=>b.missionId===p.id&&b.status==="pending"),m=document.createElement("div");m.className="bg-white dark:bg-gray-800 rounded-xl shadow-lg overflow-hidden border-l-4 border-yellow-500",m.innerHTML=`
                <div class="p-6">
                    <!-- Header com título e XP -->
                    <div class="flex justify-between items-start mb-4">
                        <div class="flex-1 pr-2">
                            <h4 class="mission-card-title text-lg font-bold mb-1 line-clamp-2">
                                ${p.titulo||p.title||"Missão sem título"}
                            </h4>
        ...
        `,o.appendChild(i),e.sort((l,m)=>{const b=I(l.completedAt||l.submittedAt);return I(m.completedAt||m.submittedAt)-b});const p=document.getElementById("completed-missions-list");e.forEach(l=>{const m=document.createElement("div");m.className="bg-white dark:bg-gray-800 rounded-xl shadow-lg hover:shadow-2xl transition-all duration-300 overflow-hidden border-l-4 border-green-500 transform hover:-translate-y-1",m.innerHTML=`
            ...
            `,p.appendChild(m)})}if(c.length>0){const i=document.createElement("div");i.id="available-missions-section",i.className="missions-section col-span-full",i.innerHTML=`
            <div class="mb-6 flex items-center justify-between">
        ...
        `,o.appendChild(i);const p=document.getElementById("available-missions-list");c.forEach(l=>{const m=document.createElement("div");m.className="bg-white dark:bg-gray-800 rounded-xl shadow-lg hover:shadow-2xl transition-all duration-300 overflow-hidden border-l-4 border-blue-500 transform hover:-translate-y-1 cursor-pointer",m.dataset.missionId=l.id,m.addEventListener("click",function(){const b=document.getElementById("mission-select");b&&(b.value=l.id,b.dispatchEvent(new Event("change"))),document.querySelector("#submit-code-btn").scrollIntoView({behavior:"smooth"});const A=document.querySelector("#submit-code-btn").closest(".bg-white");A.classList.add("highlight-pulse"),setTimeout(()=>{A.classList.remove("highlight-pulse")},2e3)}),m.innerHTML=`
            ...
            `,p.appendChild(m)})}else e.length===0&&(o.innerHTML=`
        ...
        `;return}t.forEach(o=>{o.isPenaltyReward?Y(o,e):Q(o,e)});const s=t.filter(o=>!o.isPenaltyReward&&o.status==="pending").length;if(s>0){const o=document.createElement("div");o.className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-6",o.innerHTML=`
            <div class="flex items-center">
                <div class="flex-shrink-0">
        ...
        `,e.insertBefore(o,e.firstChild)}}function Y(t,e){const s=t.type==="penalty",o=document.createElement("div");o.className=`bg-white rounded-lg shadow-md p-6 hover:shadow-lg transition-all duration-300 border-l-4 ${s?"border-red-500":"border-green-500"}`,o.innerHTML=`
        <div class="flex justify-between items-start mb-4">
            <div>
    ...
    `,e.appendChild(o)}function Q(t,e){const s={pending:"bg-yellow-100 text-yellow-800 border-yellow-300",approved:"bg-green-100 text-green-800 border-green-300",rejected:"bg-red-100 text-red-800 border-red-300"},o={pending:"Pendente - Aguardando Aprovação",approved:"Aprovada",rejected:"Rejeitada"},a={pending:"fas fa-clock",approved:"fas fa-check-circle",rejected:"fas fa-times-circle"},n=t.status==="pending",r=new Date-new Date(t.submittedAt)<6e4,c=document.createElement("div");let d="bg-white rounded-lg shadow-md p-6 hover:shadow-lg transition-all duration-300";n&&(d+=" border-l-4 border-yellow-400",r&&(d+=" ring-2 ring-yellow-300 ring-opacity-50")),c.className=d,c.innerHTML=`
        ...
        `:""}
    `,e.appendChild(c)}async function Z(t,e){try{console.log("🔄 Gerando feedback automático..."),console.log("   - Arquivos:",t),console.log("   - Contexto da missão:",e),g.show("🤖 Gerando feedback automático com IA...","info");const s=await T.analyzeSubmission(t,e);console.log("✅ Feedback recebido:",s);const o={missionTitle:e.title,files:t.map(a=>({name:a.name,size:a.size})),timestamp:new Date().toISOString()};console.log("📋 Informações da submissão:",o),M.show(s,o),s.success?s.isDemoFeedback?g.show("📚 Feedback de demonstração gerado! Configure Gemini para IA personalizada.","info"):g.show("✨ Feedback automático gerado com sucesso!","success"):g.show("⚠️ Erro ao gerar feedback automático","warning")}catch(s){console.error("❌ Erro ao gerar feedback:",s),g.show("Erro ao gerar feedback automático. Tente novamente.","error")}}async function ee(t,e,s){try{g.show("🤖 Preparando análise automática...","info");const o=s.map(c=>{const d=c.split("/").pop()||c.split("\\").pop();return{name:d,type:"text",size:0,content:`// Arquivo: ${d}
// Esta é uma análise baseada no histórico de submissão.
// Para uma análise mais detalhada, reenvie o arquivo.`}}),a={id:t,title:e,description:`Análise de submissão histórica para: ${e}`},n=await T.analyzeSubmission(o,a),r={missionTitle:e,files:s.map(c=>({name:c.split("/").pop()||c.split("\\").pop(),size:"N/A"})),timestamp:new Date().toISOString(),isHistorical:!0};M.show(n,r),n.success?g.show("✨ Análise automática concluída!","success"):g.show("⚠️ Erro ao gerar análise automática","warning")}catch{g.show("Erro ao gerar análise. Tente novamente.","error")}}window.requestFeedbackForSubmission=ee;document.addEventListener("DOMContentLoaded",z);document.addEventListener("DOMContentLoaded",function(){setTimeout(function(){const t=document.getElementById("theme-toggle");t&&!t.hasAttribute("data-backup-configured")&&(t.setAttribute("data-backup-configured","true"),t.addEventListener("click",function(n){n.preventDefault();const r=document.documentElement,c=document.getElementById("theme-icon");(r.getAttribute("data-theme")||"light")==="dark"?(r.setAttribute("data-theme","light"),c.className="fas fa-moon theme-icon-moon",localStorage.setItem("theme","light")):(r.setAttribute("data-theme","dark"),c.className="fas fa-sun theme-icon-sun",localStorage.setItem("theme","dark"))}));const e=localStorage.getItem("theme"),s=window.matchMedia("(prefers-color-scheme: dark)").matches,o=e||(s?"dark":"light");document.documentElement.setAttribute("data-theme",o);const a=document.getElementById("theme-icon");a&&(o==="dark"?a.className="fas fa-sun theme-icon-sun":a.className="fas fa-moon theme-icon-moon"),te()},100)});function te(){const t=document.getElementById("bug-report-btn"),e=document.getElementById("bug-report-modal"),s=document.getElementById("close-bug-modal"),o=document.getElementById("cancel-bug-report"),a=document.getElementById("bug-report-form");t&&t.addEventListener("click",()=>{e.classList.remove("hidden"),document.body.style.overflow="hidden"});function n(){e.classList.add("hidden"),document.body.style.overflow="auto",a.reset()}s&&s.addEventListener("click",n),o&&o.addEventListener("click",n),e.addEventListener("click",r=>{r.target===e&&n()}),a&&a.addEventListener("submit",se)}async function se(t){t.preventDefault();const e=document.getElementById("submit-bug-report"),s=e.innerHTML;try{e.innerHTML='<i class="fas fa-spinner fa-spin mr-2"></i>Enviando...',e.disabled=!0;const o=new FormData(t.target),a=localStorage.getItem("username")||"Usuário Desconhecido",n=localStorage.getItem("email")||"email@desconhecido.com",r=o.get("title"),c=o.get("description"),d=o.get("screenshot");let u="";d&&d.size>0&&(console.log("[BUG REPORT] Tamanho original:",(d.size/1024).toFixed(2),"KB"),u=await new Promise((v,y)=>{const w=new FileReader;w.onload=f=>{const h=new Image;h.onload=()=>{const i=document.createElement("canvas"),p=i.getContext("2d");let l=h.width,m=h.height;const b=1200;l>b&&(m=m*b/l,l=b),i.width=l,i.height=m,p.drawImage(h,0,0,l,m);const A=i.toDataURL("image/jpeg",.7);console.log("[BUG REPORT] Comprimida para:",(A.length/1024).toFixed(2),"KB"),v(A)},h.onerror=y,h.src=f.target.result},w.onerror=y,w.readAsDataURL(d)}));const E={title:r,description:c,userName:a,userEmail:n,url:window.location.href,screenshot:u||null};console.log("[BUG REPORT] Enviando para o backend...");const k=await fetch(`${API_URL}/api/bug-report`,{method:"POST",headers:{"Content-Type":"application/json",Accept:"application/json"},body:JSON.stringify(E)}),x=await k.json();if(console.log("[BUG REPORT] Resposta:",x),k.ok&&x.success)g.show("✅ Bug reportado com sucesso! 📧 Email enviado para pulppor@gmail.com","success"),document.getElementById("bug-report-modal").classList.add("hidden"),document.body.style.overflow="auto",t.target.reset();else throw new Error(x.message||"Erro ao enviar bug report")}catch(o){console.error("[BUG REPORT] Erro:",o),g.show("❌ Erro ao enviar bug report. Tente novamente.","error")}finally{e.innerHTML=s,e.disabled=!1}}
